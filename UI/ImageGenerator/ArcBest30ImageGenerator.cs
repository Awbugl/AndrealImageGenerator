...
internal class ArcBest30ImageGenerator
{
    ...
    internal BackGround Generate()
    {
        ...
        bg.Draw(new TextOnlyModel(Info.PlayerName, Font.Andrea72, Color.White, 345, 175),
                ...
                new ImageModel(Path.ArcaeaPartnerIcon(Info.Partner, Info.IsAwakened), 75, 130, 255),
                new PotentitalModel(Info.Potential, 175, 235));

        for (var i = 0; i < Math.Min(B30data.Best30List.Count, 30); ++i)
        {
            ...
            using var song = record.GetSongImg();
