...
internal class PotentitalComponent : IGraphicsComponent
{
    private readonly int _posX, _posY, _size;
    private readonly short _potential;

    internal PotentitalComponent(
        short potential,
        int positionX,
        int positionY,
        int size = 200)
    ...
    void IGraphicsComponent.Draw(System.Drawing.Graphics g)
    {
        ...
        temp.Draw(new ImageComponent(Path.ArcaeaRating(_potential), 21, 21, 158));
        if (_potential < 0)
        {
            temp.Draw(new StrokeTextComponent("--", Font.Exo64, Color.White, 65, 52, Color.ArcPurple, 8));
        }
        else
        {
            ...
            var lx = 5 +
                     ...
                     (ptt[2] == '1' ? 23 :
                      ptt[2] == '7' ? 25 : 37);
            var rx = 20 + (ptt[3] == '1' ? 17 : 30) + (ptt[4] == '1' ? 17 : 30);
            var posx = 98 - (lx + rx) / 2;
            string l = ptt[1] == '0' ? $"{ptt[2]}" : $"{ptt[1]}{ptt[2]}", r = $".{ptt[3]}{ptt[4]}";
